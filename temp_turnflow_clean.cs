...
public class TurnFlowUI : MonoBehaviour
{
    ...
    // BOT?N Movimiento: pasa a ActionPhase
    public void OnMovimiento()
    {
        if (selectedPlayer == null) return;
        if (!selectedPlayer.HasPlannedMovement) return;

        TurnManager.Instance.SetStateAction(); // te digo abajo c?mo
        movimientoButton.SetActive(false);
    ...
    // BOT?N Start: ejecuta movimiento + ataque + enemigos
    public void OnStart()
    {
        if (selectedPlayer == null) return;
    ...
    {
        // Bloquear input: estado ejecuci?n
        ...
        // 2) atacar con direcci?n elegida
